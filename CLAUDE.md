...
- **executor 자원 할당 — Dynamic Allocation + ratio 채택 (1번 7회 + 2번 9회 + 3번 9회 + 4번 14회 실측 검증)**: 설계 `pipeline/compaction-executor-sizing-design.md`, 보류된 C안 스켈레톤 `pipeline/examples/compaction_executor_sizing_example.py`
  ...
  - **여러 시간치 재처리 검증 (2026-09-21, 설계서 §5.5)**: 3번 22시간치 875GiB → 36대·10.4분·spill 0·error 0, 4번 6시간치 → 33대·3.1분. **같은 메모리로 되는 이유 = Spark는 한 번에 task 4개분(512MB 묶음 4개)만 메모리에 올린다** — 데이터 22배면 묶음 수(77 → 1,750)만 22배, 묶음 크기·동시 task·통신 버퍼는 같다. 늘어나는 것은 duration(천장 36대에서 **1시간치당 약 0.5분**)과 executor 디스크의 shuffle 통(22시간치 최대 37GiB/executor, 끝난 file group 통 삭제 시점에 따라 4~37GiB — **hostPath `spark-local-dir-1` 용량 미확인, 하루치 이상 재처리 전 확인**). dcu/1시간치는 0.108 → 0.081로 25% 내려감(고정 비용 묻힘) → 재처리는 여러 시간 한 번에 trigger가 싸다. 천장 아래 31·33대(공식 34·36) 미도달은 DA 순간값 또는 K8s 노드 자리 — 비용 영향 없어 보류. 대수 공식 `데이터GiB × 0.29`(= 9 task/GB × 0.13 ÷ 4), 데이터 = DataFlint input ÷ 2
...
- **후속 과제**: DAG 일괄 반영(설계서 §5.5 표: 1번 12대 16g, 2번 8대 20g, 3·4번 12대 18g, overhead 3g 공통) → 운영 duration·task error 0 확인 → executor `spark-local-dir-1` 용량·22시간치 peak 확인(하루치 재처리 대비). daily Compaction은 별건(대상 테이블 크기·구성 공유 후 시작)
