...
## 작업 5: Compaction 튜닝 (hourly) — 1번·2번 테이블 설정 확정, 3·4번 대기, DAG 미반영
...
- **executor 자원 할당 — Dynamic Allocation + ratio 채택 (7회 실측 검증)**: 설계 `pipeline/compaction-executor-sizing-design.md`, 보류된 C안 스켈레톤 `pipeline/examples/compaction_executor_sizing_example.py`
  ...
  - **2번 테이블 8회 검증 (2026-09-15~16, 설계서 §5.2)**: `instances` 제거 후 ratio 0.13이 **8대로 수렴** (24GB × 0.32 = 7.7). 12대 대비 duration +27%(1.9분)이나 dcu **−10~16%** — 1번의 16 → 12 축소와 같은 모양. **16g에서 spill 1.6~10.4GiB(시간대별 변동, 시간 비용은 안 보임) → 20g에서 0.** 확정: `instances`=init=min **8**, executor memory **20g**, 나머지 1번과 동일
  - **테이블 간 비교는 dcu/GB가 아니라 dcu/100만 row** — row 수가 같고 폭만 다르므로 GB로 나누면 좁은 테이블이 비효율적으로 보인다(2번 8대: dcu/GB +35%인데 dcu/100만 row는 1번 0.0241 vs 2번 0.0219). 같은 테이블 안의 회차 비교는 dcu/GB로 충분. GB = Compaction 후 파일 합계(DataFlint `output`)
  ...
  - **3·4번 절차**: `instances`=init=min=`ceil(시간당 GB × 0.32)`, ratio 0.13, max 36, 16g 1회 → spill이면 20g 1회. 판정: 수렴 대수, spill 0, dcu/100만 row 0.021~0.024, duration 2분 이내, 384MB 미만 파일 3개 미만 (설계서 §8.3)
  ...
  - 미확인: 메모리 97.62%(spill 0인 동안 조치 안 함), 3·4번 테이블 검증, 2번 spill의 압축 팽창 요인(row 수 차이는 확정), 운영 적용 후 duration(warm-up 없어 1.9분보다 짧을 것). ~~`maxExecutors`~~ 36 고정, ~~ratio 0.066 로그~~ `instances` 바닥으로 해소
...
- **후속 과제**: 3·4번 hourly 테이블 검증 → DAG 일괄 반영 → 운영 duration 확인. daily Compaction은 별건(대상 테이블 크기·구성 공유 후 시작)
