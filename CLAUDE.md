...
- **executor 자원 할당 — 설계 완료(3안 비교), 현재 정적 유지**: 설계 `pipeline/compaction-executor-sizing-design.md`, C안 스켈레톤 `pipeline/examples/compaction_executor_sizing_example.py`
  - **권고 순서: A 정적 12 (현재) → 55~60GB 도달 시 B Dynamic Allocation 먼저 → 부족하면 C 사전 산정.** B를 먼저 두는 이유는 구현 비용 — B는 Spark 설정 4줄에 롤백 1줄, C는 Trino 배관 + fallback + 실패 모드 5종
  - **A안이 지금 충분하다.** 고정 core는 duration이 데이터에 비례하므로 정적 12로 테이블당 **74.7GB까지 창(12분) 내 처리**. 현재 최대 42.3GB → **여유 1.77배**. 즉시 할 일은 `com_num_executor`를 12로 바꾸는 것뿐
  - **B안(DA): 반납 불가, 확보만 유효.** ① K8S는 `shuffleTracking.enabled=true` 필수인데 `shuffleTracking.timeout`이 무한대라 shuffle 보유 executor가 반납 안 됨(sort 전략이라 전 executor가 보유) ② `executorIdleTimeout` 60초 > job 90초 ③ 유휴가 executor 단위가 아니라 부분 유휴(종료 직전 C의 쓰기 43 task가 12 executor에 분산 → executor당 3.6/4 slot). **`initialExecutors` 기본값이 0이라 반드시 12로 명시**해야 함. 확보는 10~20초 소요(backlog 4초 + pod 5~15초)
  - **C안(사전 산정)**: 당초 6개 값을 동적화하려 했으나 **`num-executors` 하나로 좁혀졌다**. `num_executors = clamp(ceil(총 크기GB × 0.32), 4, MAX)`, **C=0.32 확정**. 산정 위치는 `compaction_specs` 내부(테이블별 try/except로 실패 격리). 입력은 `.files`가 아니라 `.partitions` **범위** 조회. 파티션 값 변환은 **naive datetime**(`ts`가 `timestamp_ntz`) — 2026-08-11 13:00 → 496237 (Spark UI 실측 일치). 기존 `com_num_executor`는 **fallback으로 유지**. **선행 조건: mapped task 전환**
  - `MAX_EXECUTORS`는 B·C 공통 미확정(K8S quota 필요, 잠정 32). **상한에 걸리면 조치 신호** — K8S에 여유가 없으면 executor를 늘려도 pod Pending으로 duration이 늘어 무의미
  - **daily는 판단이 다르다** — 30~60분 job이라 `executorIdleTimeout` 60초가 전체의 2~3%에 불과해 DA 반납이 실제로 일어날 수 있다. daily 튜닝 후 별도 판단
  - 미확인: Spark 4.1.1 DA 기본값(`spark.apache.org` 차단으로 서드파티 확인), Trino `$partitions` 타입, manifest pruning, 시간대별 데이터 편차
