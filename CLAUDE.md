...
- **⚠️ 실측 스택이 문서와 다름 (§5.0.2)**: driver Pod의 jar가 `iceberg-spark-runtime-**3.5_2.12**-1.10.1`, `aws-java-sdk-bundle-**1.12.262**`(= Hadoop 3.3.4 고정 버전, **SDK v1**) → **실제 런타임은 Spark 3.5 / Scala 2.12 / Hadoop 3.3.4로 보인다.** 문서 전반의 "Spark 4.1.1" 기재 정리 필요. **전환 분석에는 영향 없음** — Iceberg `spark/v3.5`·`spark/v4.0` 모듈의 삭제 로직과 Hadoop 3.3.4·3.4.1의 S3A delete 경로, `fs.s3a.*` 기본값이 모두 동일함을 소스 대조로 확인
- **jar 조치**: `iceberg-spark-runtime`은 `iceberg-aws`를 포함하지만 **AWS SDK는 미포함**(`spark/v3.5/build.gradle:241`). `iceberg-aws-bundle-1.10.1.jar`를 **추가**하면 되고, **기존 `aws-java-sdk-bundle`(v1)은 제거하지 말 것** — S3A가 쓴다. **v1(`com.amazonaws.*`)과 v2(`software.amazon.awssdk.*`)는 패키지가 달라 공존 가능**
