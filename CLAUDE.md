...
- **진행 상태 (2026-09-13)**: 개발 클러스터에서 `backup` 통과(건수 일치). `load`는 `[Oracle 에 키 없는 row]` 로그 직후 `AnalysisException UNRESOLVED_COLUMN t.tmp_id`로 실패했었고, **원인은 로컬 재현으로 확정** — 신규 테이블이 `tmp_id` 없이 재생성된 상태에서 PR #64 이전 코드를 돌리면 INSERT와 `[load 신규 vs 임시]` 검수가 조용히 통과한 뒤 마지막 `tmp_id 불일치` 쿼리에서 정확히 같은 예외(`UNRESOLVED_COLUMN.WITH_SUGGESTION`, 제안 목록에 `t.tmp_id` 없음)가 난다. 대안 가설(`TMP_ID` 대문자 DDL)은 INSERT 단계에서 `t.TMP_ID`로 실패해 서명이 다르므로 배제. 현재 코드(PR #64)는 두 경우 모두 처리 — `tmp_id` 부재는 시작 시 `require`로 즉시 실패, 대문자는 전 구간 통과. 개발에서는 `DESCRIBE`로 `tmp_id` 부재만 확인하면 된다
- **로컬 재현으로 추가 확인 (2026-09-13, Spark 3.5.8 / Iceberg 1.10.1, Oracle은 Derby 인메모리 대역)**: ①정상 경로 전 구간 통과 — NOT NULL 컬럼에 `COALESCE(o.tmp_id, '')` INSERT 허용, 완전 중복 row·NULL array·NaN 모두 `EXCEPT ALL` 검수 통과 ②**`SHOW CREATE TABLE` 출력을 그대로 CREATE에 쓰면 `'sort-order'`·`'current-snapshot-id'` 줄은 에러 없이 무시된다** — Sort Order 없는 테이블이 만들어지고 `load` 검수도 못 잡는다. `WRITE ORDERED BY` 후 `SHOW CREATE TABLE`에 `'sort-order'`가 다시 보이는지 확인 (절차서 반영) ③`[Oracle 에 키 없는 row]`에는 `DtFrom`~`DtTo` 밖의 Oracle row도 들어간다 — 범위가 대상 데이터 전체를 덮는지 확인 필요(절차서 반영). 재현 환경은 scratchpad라 세션 종료 시 사라짐: Spark 배포판 + `iceberg-spark-runtime-3.5_2.12-1.10.1.jar`, JDK 17(apt), 컴파일은 배포판의 `scala-compiler` jar(`java -cp "jars/*" scala.tools.nsc.Main -usejavacp`), Oracle 대역은 배포판 내장 Derby(`jdbc:derby:memory:`)
...
- **다음 단계**: 개발에서 `DESCRIBE`로 `tmp_id` 부재 확인 → backup → 수동 DDL(`WRITE ORDERED BY` 후 `SHOW CREATE TABLE`로 `sort-order` 확인) → load 전 구간 통과 → 운영 적용 (Airflow 중지 → backup → DDL → load → Trino 확인 → 재개 → 며칠 뒤 임시 `DROP ... PURGE`) → 다른 테이블에 같은 절차 반복
