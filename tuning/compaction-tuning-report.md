...
| `executor.instances` / `initialExecutors` / `minExecutors` | **세 값 동일.** 시간당 데이터GB × 0.32 — 1번 12, 2번 8 | **테이블별** |
| `maxExecutors` | 36 | 공통 |
| executor memory | 1번 16g, 2번 20g | **테이블별** |
...
| 항목 | 내용 |
|------|------|
| 나머지 테이블 2개(3·4번) 검증 | 테이블당 1~2회. 시작 대수 = 시간당 GB × 0.32, 16g에서 spill이 나면 20g |
