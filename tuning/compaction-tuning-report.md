...
## 5. executor 자동 산정 — 설계 완료, 도입 보류

데이터 증가 시 소요시간이 늘어 실행 창(12분) 제약을 넘는 문제에 대비해 executor 수를 데이터 양에 따라 산정하는 방식을 설계했다. **현재 도입은 불필요하다.**

**도입 시점 판단**

| 데이터 (테이블당) | 정적 12개 | 자동 산정 |
|------------------|----------|----------|
| 42.3GB (현재 최대) | 6.8분 | 6.0분 |
| 60GB | 9.6분 | 6.0분 |
| 74.7GB | **12.0분 (창 초과)** | 6.0분 |

- 정적 12개로 테이블당 74.7GB까지 창 내 처리 가능. 현재 대비 **여유 1.77배**
- 도입 시점: 테이블당 **55~60GB** 도달 시
- 현재 산정값도 12~14개로 정적값과 거의 동일
...
**즉시 적용** — `com_num_executor`를 12로 변경. 이것만으로 1장의 리소스 절감(−47%)을 확보한다.
