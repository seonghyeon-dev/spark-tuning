...
| daily Compaction 점검 | 888GB에 30~60분. **daily DAG이 hourly 테이블을 다시 도는지부터 확인** — 재처리 설계서는 두 DAG의 대상 테이블 그룹이 다르다고 기술. 다시 돈다면 `rewrite-all=false`로 읽는 양이 지연 도착분 수준으로 감소(소스 확인). 아니라면 하루치 원본 전체 재작성이 정상이며 별도 튜닝 대상 |
