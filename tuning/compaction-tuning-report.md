...
- 실행 창은 매시 45분~정각(12분). 여유 3분 → 6분.
- 절감 리소스는 5분 주기 append Job과의 경합 감소로 이어진다.
...
Compaction은 파티션별로 파일을 묶은 file group 단위로 처리한다. `max-concurrent-file-group-rewrites`가 동시 처리 group 수를 정한다.

**원인** — group 7개를 2개씩 처리해 4회차로 분할. 각 회차는 느린 쪽이 끝날 때까지 대기한다.
...
1·4회차가 데이터 15%에 소요시간 37%를 사용. CPU 64 core 중 58%가 유휴.
...
**원인** — 2.1 조치 후에도 유휴 CPU 25~29%.

**조치** — executor 16 → 12. 축소 한계 확인을 위해 3개 지점을 측정했다.
...
8에서 CPU 33% 감소 대비 소요시간 56% 증가로 리소스 비용이 반등. 12가 하한이다. disk spill은 3개 지점 모두 0이다.
...
출력 파일 크기는 `target-file-size-bytes`(512MB)만으로 결정된다. Iceberg가 shuffle partition 수를 직접 산정하므로 두 Spark 설정은 개입하지 않는다.
...
| `rewrite-all` | true | 입력 파일이 전부 대상이므로 false와 처리량 동일 |
| `executor cpu / memory` | 4 core / 16GB | 메모리 사용률 89~94%, disk spill 0 |
| `partial-progress.enabled` | 기본값 (false) | 변경 시 snapshot 증가로 보존 정책·재처리 DAG에 영향 |
...
목표 512MB 대비 평균 509MB. 9회 전부 동일 수준을 유지했다.

최소 크기 300~415MB는 조치하지 않는다.

- **원인** — `col_a=D` 파티션이 시간당 600~830MB. Iceberg가 512MB 기준으로 나누므로 623MB는 311MB 파일 2개가 된다.
- **범위** — 파일 75개 중 2개, 데이터 37GB 중 0.9GB(2.4%).
- **해소 조건** — 목표 파일 크기를 830MB 이상으로 상향. 전체 파티션에 적용되어 스키마 설계 결정과 충돌한다.
...
| daily Compaction 점검 | 888GB에 30~60분. 소요시간이 데이터 양에 비례하므로 전체 재작성 여부 확인 |
