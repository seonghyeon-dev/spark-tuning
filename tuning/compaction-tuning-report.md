...
데이터 증가 시 소요시간이 늘어 실행 창(12분) 제약을 넘지 않도록 executor 수를 데이터 양에 따라 산정한다.

| 항목 | 내용 |
|------|------|
| 산정식 | `executor = 총 크기(GB) × 0.32`, 하한 4 / 상한 미확정 |
| 계수 근거 | 2.2의 3개 지점 측정 (37.3GB에서 12개가 리소스 비용 최저) |
| 입력 측정 | Iceberg `.partitions` 메타데이터 조회 (Trino) |
| 조회 실패 시 | 기존 정적값으로 대체 |
| 적용 범위 | hourly 전용. daily는 별도 측정 필요 |
| 구현 | `pipeline/examples/compaction_executor_sizing_example.py` |
...
현재 데이터 범위(36~42GB)에서 산정값이 12~14개로 좁아 즉각 이득은 작다. 정적 12개로 운영하며 도입을 미루는 선택도 가능하다.
