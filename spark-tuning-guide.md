...
EXECUTOR_CORES = 4
PARALLELISM_FACTOR = 2        # 코어 대비 태스크 배수 (파이프라이닝 여유)
...
2. `× PARALLELISM_FACTOR(2)` = 코어당 2개 태스크를 대기시켜 파이프라이닝 여유 확보
3. `/ executor-cores(4)` = Executor 1개가 동시 처리하는 태스크 수 반영

테스트 데이터 적용:
```
읽기 파티션 = ceil(8080 / 128) = 64
num_executors = ceil(64 × 2 / 4) = 32
```

| 산정 방식 | 결과 | 총 코어 수 | 비고 |
|----------|------|-----------|------|
| 기존 (70MB) | 116 | 464 | 과다 할당 |
| 개선 (128MB + cores 반영) | 32 | 128 | 읽기 파티션 대비 적정 |
| 최소 (1라운드 처리) | 16 | 64 | 태스크 수 = 코어 수 |

> ⚠️ **개선 필요 (벤치마크 검증 대기)**
> 개선 산정식은 `spark.sql.files.maxPartitionBytes`(128MB)와 `executor-cores`를 반영하여 이론적 근거가 있으나, 아직 벤치마크를 통한 최적값 검증이 필요합니다.
>
> **벤치마크 테스트 케이스**:
> | 케이스 | num-executors | 산정 근거 | 총 코어 수 |
> |--------|---------------|----------|-----------|
> | A | 16 | 최소 1라운드 처리 (`ceil(8080/128/4)`) | 64 |
> | B | 24 | 16 × 1.5 (여유 50%) | 96 |
> | C | 32 | 개선 산정식 (`PARALLELISM_FACTOR=2`) | 128 |
> | D | 60 | 기존 테스트값 (비교 기준선) | 240 |
>
> **확인 사항**: 소요 시간이 수렴하는 지점 = 비용 대비 최적값. 10분 주기 배치이므로 목표 소요 시간(예: 3분 이내)을 정하고, 그 안에 들어오는 최소 Executor 수를 찾는 것이 합리적이다.
...
>
> avro → Iceberg append 워크플로우에서 **읽기 파티션**은 항상 적용되며, **셔플 파티션**은 Iceberg 테이블에 파티셔닝 컬럼이 있어 데이터 재분배가 필요한 경우에만 적용된다.
...
> ⚠️ **개선 필요**
> 현재 70으로 설정되어 있으나 명확한 근거가 부족합니다. 먼저 **실제로 shuffle이 발생하는지 확인**하는 것이 선행되어야 합니다.
>
> **1단계: shuffle 발생 여부 확인**
> - Spark UI → Stages 탭에서 shuffle read/write 크기가 0보다 큰 Stage가 있는지 확인
> - shuffle이 없거나 미미하면 이 옵션은 실질적 영향이 없으므로 기본값 200 유지로 충분
>
> **2단계: shuffle이 발생하는 경우 벤치마크**
> | 케이스 | 값 | 비고 |
> |--------|-----|------|
> | A | 70 (현재) | 비교 기준선 |
...
>
> **산정 기준 참고**:
> - `총 shuffle 데이터 크기(MB) / 100~200 = 적정 파티션 수`
> - `executor 수(N) × executor-cores(4) × 2 = 적정 파티션 수`
...
| `num-executors` | `총 크기/70MB+1` → 개선: `ceil(총 크기/128MB×2/cores)` | `2` | ⚠️ 테스트 기반 → 벤치마크 검증 대기 | 기존 과다 할당. 개선 산정식 제안됨 |
...
#### 테스트 1 (높음): num-executors 최적값 벤치마크

**목표**: 개선 산정식(`ceil(총 크기 / 128MB × 2 / executor-cores)`)의 적정성 검증
...
| B | 24 | 16 × 1.5 (여유 50%) | 96 |
| C | 32 | 개선 산정식 (`PARALLELISM_FACTOR=2`) | 128 |
| D | 60 | 기존 테스트값 (비교 기준선, 1.1분) | 240 |

**확인 사항**: 소요 시간이 수렴하는 지점 = 비용 대비 최적값. 10분 주기 배치이므로 목표 소요 시간(예: 3분 이내)을 정하고, 그 안에 들어오는 최소 Executor 수를 찾는다.

---

#### 테스트 2 (중간): shuffle 발생 여부 확인 및 shuffle.partitions 검증

**선행 조건**: Spark UI → Stages 탭에서 shuffle read/write 크기가 0보다 큰 Stage가 있는지 확인

| 상황 | 대응 |
|------|------|
| shuffle이 없거나 미미함 | 이 옵션은 영향 없음. 기본값 200 유지 |
| shuffle이 발생함 | 아래 케이스 테스트 |

**테스트 케이스** (shuffle 발생 시):
...
```
1단계: Spark UI에서 shuffle 발생 여부 확인
       → shuffle.partitions 테스트 필요성 판단

2단계: num-executors 테스트 (A→B→C→D)
       → 소요 시간이 수렴하는 지점 = 최적값

3단계: parallelismFirst 테스트 (true vs false)
       → Iceberg 파일 품질 비교
```
