...
## Spark file splits read vs Trino Splits 비교

Trino에서 `EXPLAIN ANALYZE`를 실행하면 Fragment [source]의 마지막 줄에 **Splits** 값이 표시된다. 이 값은 Spark의 file splits read와 개념적으로 동일한 역할이다.

### 공통점

둘 다 **result data files를 병렬로 읽기 위해 나눈 조각 수**를 의미한다. Iceberg가 선정한 동일한 파일을 대상으로 하므로, 읽는 데이터 총량(total data file size)은 같다.

### 절대값이 다른 이유

같은 파일을 읽더라도 한 split에 얼마나 담느냐는 엔진 설정에 따라 다르다.

| 설정 | Spark | Trino |
|------|-------|-------|
| split 크기 결정 설정 | `spark.sql.files.maxPartitionBytes` (기본 128MB) | Iceberg connector의 `max-split-size` 등 |
| 실측 평균 split 크기 | ~50 MB | ~100 MB |

Trino가 Spark보다 약 2배 큰 split을 사용하므로, 동일한 데이터에 대해 Trino Splits 수는 Spark file splits read의 약 절반이 된다.

### 전략 간 비교에는 둘 다 유효하다

| 메트릭 | A안 | B안 | 감소율 |
|--------|-----|-----|--------|
| Spark file splits read | 107 | 47 | 56% |
| Trino Splits | 53 | 23 | 57% |

절대값은 다르지만 **전략 간 감소 비율은 동일**하다. 따라서 Trino의 Splits 값도 Data Skipping 효과를 비교하는 지표로 활용할 수 있다 — Splits가 적을수록 읽는 데이터가 적다는 뜻이다.
