...
따라서 WHERE 절에 없더라도, 낮은 Cardinality로 데이터 조직화에 적합한 컬럼은 파티션 키로 유효하다.
...
SET TBLPROPERTIES (
  ...
  -- array 타입 7개 컬럼은 min/max 통계 의미 없음
...
| Write Ordering | 사용/미사용, 컬럼 선택 | `sort_a, sort_b, sort_c` (사용) | ✅ 벤치마크 검증 | WHERE 절의 3개 필터 컬럼을 모두 정렬에 반영. 파티션 프루닝(`day(ts)`)만으로 부족 |
