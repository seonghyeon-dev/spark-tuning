...
스키마 설계의 핵심 목표는 이러한 다차원 필터 조회에서 **불필요한 파일 읽기를 최소화**(Data Skipping)하는 것이다. 파티션 프루닝은 `day(ts)` + `par_a` + `par_b`에서 작동하며, **sort_a, sort_b, sort_c의 Write Ordering + Data Skipping이 추가 최적화 수단**이다.
...
> **TABLE_A**: 일일 ~35,712 파일로 ❌ 위험 구간. 정기 Compaction이 필수이며, 이미 운영 계획에 포함되어 있다 (1시간 + 1일 단위 배치).

**Compaction 실측 결과** ✅ 벤치마크 검증
...
| `par_b` | identity | ✅ 벤치마크 검증 | par_a별 42~72개, 전체 조합 248개. WHERE 절에 항상 포함되어 파티션 프루닝 유효. 파티션 분포 불균등(Skew) 존재하나 Compaction으로 관리. 정기 Compaction 필수 |
...
**TABLE_A 판단**: 파티션 프루닝(`day(ts)` + `par_a` + `par_b`) 후에도 `sort_a`, `sort_b`, `sort_c`로 추가 필터링이 필요하며, 반복 조회가 발생한다. → **Write Ordering 필요** ✅
...
|---------|--------|-----------|------------|
| 850GB / 36MB (Compaction 전) | 23,789개 | 23,789회 | 높음 |
| 850GB / 464MB (Compaction 후) | 1,834개 | 1,834회 | **13배 감소** |
| 850GB / 1GB | ~850개 | ~850회 | 낮음 (단, 병렬성 저하) |
...
> **TABLE_A 적용**: 일 적재량 ~936GB, S3(MinIO) 스토리지, Compaction 운영. Compaction 후 평균 파일 크기 ~464MB로 512MB 목표에 근접하며, 1,834개 파일로 메타데이터 부하 적정. **512MB가 적합**하다.
...
| 일일 파일 수 (Compaction 전) | 144 × 248 = **35,712개/일** → ❌ 위험 구간 |
...
| 일일 파일 수 | Compaction 전: 35,712개 ❌ → **Compaction 후: ~1,834개** ✅ |
| Compaction 부담 | 높음 — 1시간 + 1일 단위 필수 (실측 검증 완료) |
| 프루닝 정밀도 | **최대** — `WHERE par_b = 'x'` 시 정확히 1개 파티션만 scan |
| Skew 영향 | 있음 — 하위 ~190개 조합이 small file, Compaction으로 관리 |
...
| 예상 일일 파일 수 (Compaction 전) | 35,712개 | **19,440개** | 46% 감소 |
...
| Compaction 전 파일/일 | 35,712 ❌ | 19,440 ⚠️ | 9,216 ⚠️ |
...
| par_b 파티션 | `identity`/`truncate`/`bucket` | `identity` (A안 권장) | ✅ 벤치마크 검증 | 조합 248개, WHERE 항상 포함. Compaction 부담 시 truncate(3) B안 또는 bucket(16) C안 (7.1절 참조) |
