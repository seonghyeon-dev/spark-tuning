...
- [4. Bucketing](#4-bucketing) — bucket 트랜스폼, 적용 시나리오
...
- [7. 설계 결정 요약](#7-설계-결정-요약) — **추천 전략**, 의사결정 매트릭스, DDL 예시
- [8. 용어집](#8-용어집)
- [9. 참고 자료](#9-참고-자료)
...
| 4 | sort_a | string | Write Ordering 1순위 |
| 5 | sort_b | string | Write Ordering 2순위 |
| 6 | sort_c | string | Write Ordering 3순위 |
| 7 | id_1 | string | 식별자 |
| 8 | val_1 | double | 값 |
| 9 | val_2 | double | 값 |
...
| sort_a | Write Ordering 1순위 | 31,697 | WHERE 절 등가 조건 |
| sort_b | Write Ordering 2순위 | 25,820 | WHERE 절 등가 조건 |
| sort_c | Write Ordering 3순위 | 563,691 | WHERE 절 등가 조건 |
...
TABLE_A 실측값:
  ...
  ─────────────────────────────────────────────────
  144 × 248 = 35,712 파일/일 (Compaction 전) ← ⚠️ 주의, 정기 Compaction 필수
...
> **TABLE_A**: 일일 ~35,712 파일로 ❌ 위험 구간. 정기 Compaction이 필수이며, 이미 운영 계획에 포함되어 있다.
...
| 60위 이후 | 0.001GB 이하 | 전부 1개 | ❌ small file, Compaction 필수 |

> **진단**: 248개 조합으로 ⚠️ 주의 구간. 상위 50개가 대부분의 데이터를 차지하여 **Skew가 존재**하며, 하위 ~190개 조합이 1GB 미만으로 **small file 주의** (Compaction 필수) 상태이다.
...
| sort_a | 31,697 | 수만 | bucket 필수 | ❌ 고 Cardinality, identity 불가. Write Ordering에 적합 |
| sort_b | 25,820 | 수만 | bucket 필수 | ❌ 고 Cardinality, identity 불가. Write Ordering에 적합 |
| sort_c | 563,691 | 수십만 | bucket 필수 | ❌ 매우 높은 Cardinality, identity 불가. Write Ordering에 적합 |

> **결론**: 3번째 파티션 키로 par_b identity가 현실적인 유일한 선택지이다. par_c, par_d는 identity 사용이 불가능하며, sort_a/sort_b/sort_c는 고 Cardinality로 identity 파티션이 불가능하여 Write Ordering + Data Skipping으로 최적화한다. 하위 ~190개 조합의 small file은 **정기 Compaction으로 관리**한다.
...
| Write Ordering | `sort_a, sort_b, sort_c ASC NULLS FIRST` | ✅ 벤치마크 검증 | 조회 패턴 분석 결과: sort_a(빈도 높음) → sort_b(빈도 높음) → sort_c(보조). WHERE 절의 3개 필터 컬럼을 모두 정렬에 반영 |
...
**Cardinality와 Write Ordering 효과의 관계**
...
**TABLE_A 판단**: par_a Cardinality 4 (identity 최적), par_b 조합 248개 (identity 유지, Compaction 필수). → **Bucketing 불필요** ✅
...
Z-ordering은 Compaction 시 적용하는 **추가 최적화**이다. Write Ordering으로 충분하면 불필요하다.

| 질문 | 예 → | 아니오 → |
|------|------|---------|
| Write Ordering의 2순위 컬럼이 단독 필터로 자주 사용되는가? | **Z-ordering 후보** | Write Ordering으로 충분 |
...
**TABLE_A 판단**: sort_a, sort_b, sort_c 3개 컬럼 체제에서 sort_b, sort_c 단독 필터 빈도가 아직 충분히 높지 않다. → **현재 Write Ordering 유지, 단독 필터 빈도 추가 확인 후 결정** 📘 일반적 관행
...
TABLE_A의 조회 패턴이 **다차원 필터**(ts + sort_a + sort_b + sort_c)인 경우, Z-ordering은 Write Ordering 대비 이점이 있다:
...
**TABLE_A에서의 적용 판단** 📘 일반적 관행

| 판단 기준 | 현재 상황 |
|----------|----------|
| 조회 패턴 다양성 | 다차원 필터 → Z-ordering 후보 |
| Write Ordering과의 관계 | 쓰기 시 Write Ordering, Compaction 시 Z-ordering으로 전환 가능 |
...
| Compaction 운영이 가능한가? | 운영 불가 → 쓰기 시 정렬 (Write Ordering) | 운영 가능 → Compaction 시 Z-ordering |
...
| 연산 비용 | Z-curve 계산(비트 인터리빙) + 전체 데이터 정렬 → Write Ordering 대비 Compaction 시간 20~50% 증가 |
...
| Write Ordering 2순위 컬럼 (min/max 범위 넓음) | ✅ **효과적**. min/max로 못 거른 파일을 추가 skip |
...
**TABLE_A 적용 검토** 📘 일반적 관행

| 컬럼 | Bloom Filter | 이유 |
|------|-------------|------|
| `sort_a` | 검토 (2단계) | Write Ordering 1순위이므로 min/max로 대부분 skip. 추가 효과 제한적일 수 있음 |
| `sort_b` | 검토 (2단계) | Write Ordering 2순위. min/max 범위가 넓어 등가 필터 시 Bloom Filter 효과 기대 |
| `sort_c` | **검토 권장** | Write Ordering 3순위. min/max 범위가 가장 넓어 등가 필터 시 Bloom Filter 효과 최대 |
...
현재까지 확인된 정보를 종합하면, TABLE_A의 핵심 설계 결정은 **par_b 파티션의 유지 여부**로 귀결된다. 나머지 설정(`day(ts)`, `par_a`, Write Ordering `sort_a/sort_b/sort_c`, `range` 모드)은 모든 안에서 동일하다.

**현재 상황 정리**

| 항목 | 현황 |
|------|------|
| 파티션 프루닝 확정 | `day(ts)` ✅, `par_a` ✅ |
| 파티션 프루닝 미확정 | `par_b` ⚠️ (UI WHERE 절 포함 여부 확인 필요) |
| Write Ordering | `sort_a`, `sort_b`, `sort_c` — WHERE 절의 3개 필터 컬럼, 모든 안에서 동일 |
| 일일 파일 수 (현행) | 144 × 248 = **35,712개/일** → ❌ 위험 구간 |
| par_b Cardinality | 248개 (par_a별 42~72), 상위 50개가 대부분 데이터 차지, Skew 존재 |
| Compaction | 현행 기준 **필수**, 이미 운영 계획 포함 |

---

**A안 (권장): 현행 유지 — par_b가 WHERE 절에 포함되는 경우**
...
| 항목 | 내용 |
|------|------|
| 조건 | par_b가 UI WHERE 절에 포함 확정 |
| 프루닝 체인 | `day(ts)` → `par_a` → `par_b` → sort_a/b/c Data Skipping |
| 프루닝 효과 | 1일 데이터 → 1/4(par_a) → 1/248(par_b) → sort Data Skipping |
| 일일 파일 수 | 35,712개 (❌ 위험, Compaction 필수) |
| Compaction 부담 | 높음 — 시간당 또는 최소 일 1회 필수 |
| 장점 | **읽기 성능 최대**. par_b 프루닝으로 scan 대상을 1/248까지 축소 |
| 단점 | 운영 복잡도 높음. Compaction 미운영 시 메타데이터 부하 급증 |
| 적합한 경우 | par_b 프루닝 확정 + Compaction 안정 운영 가능 |

> **핵심 판단**: par_b 프루닝이 가능하면, 35,712개/일의 운영 비용을 감수하더라도 읽기 성능 이득이 크다. 248개 파티션 중 하나만 scan하는 것과 전체를 scan하는 것은 **수십~수백 배 차이**다.

---

**B안 (차선): par_b 파티션 제거 — par_b가 WHERE 절에 포함되지 않는 경우**
...
| 항목 | 내용 |
|------|------|
| 조건 | par_b가 UI WHERE 절에 미포함 확정, 또는 운영 단순성 최우선 |
| 프루닝 체인 | `day(ts)` → `par_a` → sort_a/b/c Data Skipping |
| 프루닝 효과 | 1일 데이터 → 1/4(par_a) → sort Data Skipping |
| 일일 파일 수 | 144 × 4 = **576개** (✅ 안전) |
| Compaction 부담 | **최소** — 일 1회 정도로 충분 |
| 장점 | 운영 단순. 파일 수 현행 대비 **98% 감소**. small file 문제 거의 해소 |
| 단점 | par_b 프루닝 불가. par_a 파티션 내 모든 파일을 scan 후 sort Data Skipping에만 의존 |
| 적합한 경우 | par_b 프루닝 불가 확정, 또는 Compaction 인프라 제한적 |

> **핵심 판단**: par_b가 WHERE에 없다면, 파티션으로 유지하는 것은 35,712개 파일의 운영 비용만 발생시키고 프루닝 이점이 없다. 데이터 조직화는 `day(ts)` + `par_a`만으로도 충분하다.

**B안 적용 시 DDL 변경**

```sql
-- Partition Evolution: par_b 제거 (메타데이터만 변경, 기존 데이터 유지)
ALTER TABLE catalog.db.TABLE_A DROP PARTITION FIELD par_b;

-- 기존 데이터도 새 파티션 구조로 재정리하려면 Compaction 실행
CALL catalog.system.rewrite_data_files(table => 'db.TABLE_A');
```

---

**C안 (절충): par_b → bucket 전환 — 프루닝은 필요하나 파일 수를 줄여야 하는 경우**
...
| 항목 | 내용 |
|------|------|
| 조건 | par_b가 WHERE 절에 포함되나, 35,712개/일의 운영 비용이 부담 |
| 프루닝 체인 | `day(ts)` → `par_a` → `bucket(par_b)` → sort_a/b/c Data Skipping |
| 프루닝 효과 | 1일 데이터 → 1/4(par_a) → 1/16(bucket) → sort Data Skipping |
| 일일 파일 수 | 144 × 4 × 16 = **9,216개** (⚠️ 주의, 현행 대비 74% 감소) |
| Compaction 부담 | 중간 — 시간당 권장 |
| 장점 | par_b 등가 프루닝 유지 (1/16 scan). 파일 수 현행 대비 대폭 감소 |
| 단점 | identity 대비 프루닝 정밀도 저하 (1/248 → 1/16). bucket 해시 특성상 Skew 완화되나 범위 조회 불가 |
| 적합한 경우 | par_b 프루닝 필요 + 파일 수 감소 필요 + Compaction 부담 줄이고 싶을 때 |

> **핵심 판단**: identity(248개) → bucket(16개)으로 전환하면 프루닝 정밀도는 1/248에서 1/16으로 낮아지지만, 파일 수가 74% 줄어 운영 부담이 크게 감소한다. par_b Skew(상위 50개 집중)도 해시 분배로 완화된다.
...
| 항목 | A안 (현행 유지) | B안 (par_b 제거) | C안 (bucket 전환) |
|------|---------------|----------------|-----------------|
| 전제 조건 | par_b WHERE 포함 ✅ | par_b WHERE 미포함 | par_b WHERE 포함 ✅ |
| 일일 파일 수 | 35,712 ❌ | 576 ✅ | 9,216 ⚠️ |
| par_b 프루닝 | 1/248 (최대) | 불가 | 1/16 (축소) |
| 읽기 성능 | ★★★★★ | ★★★ | ★★★★ |
| 운영 복잡도 | ★★★★★ (높음) | ★ (낮음) | ★★★ (중간) |
| Compaction 필요도 | 필수 (시간당) | 선택 (일 1회) | 권장 (시간당) |
| Skew 영향 | 있음 (하위 190개 small file) | 없음 | 완화 (해시 분배) |
| 전환 비용 | 없음 (현행) | 낮음 (DDL 1줄) | 낮음 (DDL 2줄) |

**결론: 현재 정보 기준 판단 방향**

```
par_b가 UI WHERE 절에 포함되는가?
  ├─ YES → Compaction 안정 운영 가능한가?
  │         ├─ YES → A안 (현행 유지, 최대 읽기 성능)
  │         └─ NO  → C안 (bucket 전환, 절충)
  └─ NO  → B안 (par_b 제거, 운영 단순화)
```

> **참고**: 모든 안에서 Partition Evolution으로 무중단 전환이 가능하다 (메타데이터만 변경). 따라서 **A안으로 시작 → 운영 부담이 크면 C안 또는 B안으로 전환**하는 단계적 접근도 유효하다.
...
| par_a 파티션 | `identity`/`bucket` | `identity` | ✅ 벤치마크 검증 | Cardinality 4, WHERE 항상 포함 → 프루닝 유효 |
| par_b 파티션 | `identity`/`bucket`/제거 | `identity` (⚠️ 검토 중) | ✅ 벤치마크 검증 | 조합 248개. WHERE 포함 시 A안, 미포함 시 B안 (7.1절 참조) |
| Write Ordering | 사용/미사용, 컬럼 선택 | `sort_a, sort_b, sort_c` (사용) | ✅ 벤치마크 검증 | WHERE 절의 3개 필터 컬럼을 모두 정렬에 반영 |
| Distribution Mode | `none`/`hash`/`range` | `range` | 📘 일반적 관행 | Write Ordering 사용 시 range 필수 |
| Z-ordering | 미적용/Compaction 시 적용 | 미적용 (2단계 검토) | 📘 일반적 관행 | sort_b, sort_c 단독 필터 빈도 추가 확인 후 결정 |
| Column Metrics | `none`/`truncate`/`full` | 기본값 유지 + array는 `none` | 📘 일반적 관행 | 필터 대상 컬럼 통계 수집 필수 |
| Bloom Filter | 컬럼별 on/off | sort_b, sort_c 검토 | 📘 일반적 관행 | Write Ordering 2·3순위 컬럼의 등가 필터 보완 |
...
```sql
-- Write Ordering 2·3순위 컬럼에 Bloom Filter 적용 (검토 후)
ALTER TABLE catalog.db.TABLE_A
SET TBLPROPERTIES (
...
| **Bucket Transform** | Murmur3 해시로 컬럼 값을 N개 버킷에 분배하는 파티션 트랜스폼. 고 Cardinality 컬럼에 적합 |
| **Cardinality** | 컬럼의 고유값(distinct value) 수. 파티션 트랜스폼 선택의 핵심 기준 |
| **Compaction** | Iceberg `rewrite_data_files` 프로시저. small file 병합, 정렬(Z-ordering 포함) 적용 |
| **Small File 문제** | 파티션이 과다하거나 배치 빈도가 높아 작은 파일이 다수 생성되는 현상. 읽기 성능 저하의 주요 원인 |
