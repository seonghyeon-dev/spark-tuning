...
| 절 | 사용 컬럼 | 빈도 | 조건 유형 | 비고 |
|----|----------|------|----------|------|
| WHERE | ts | **필수** | `date(ts) = ...`, `date(ts) IN (...)`, 시간 범위 | 날짜 또는 시간값. `day(ts)` 파티션으로 프루닝 |
| WHERE | par_a | **필수** | 등가 (`=`, `IN`) | 파티션 프루닝 유효 |
| WHERE | par_b | **필수** | 등가 (`=`, `IN`) | 파티션 프루닝 유효. 단, 파티션 분포 불균등 (Skew) 존재 |
| WHERE | sort_a | **필수** | 등가 (`=`, `IN`) | Sort Order 1순위 |
| WHERE | sort_b | **필수** | 등가 (`=`, `IN`) | Sort Order 2순위 |
| WHERE | sort_c | **필수** | 등가 (`=`, `IN`) | Sort Order 3순위 |
...
range 모드에서 par_b Sort Order 1순위 시 Data Skipping 동작:
  ...
  → 배치가 누적되어도 배치당 1~2개 파일만 매칭, Compaction 전에도 유효
```
