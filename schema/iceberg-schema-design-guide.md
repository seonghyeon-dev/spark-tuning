...
| sort_b | Write Ordering 2순위 (기존) | = par_b, 파티션 프루닝으로 대체 → Write Ordering에서 제외 권장 |
| sort_c | Write Ordering 3순위 → 2순위 | WHERE 절 등가 조건 |
...
| WHERE | sort_b (=par_b) | 높음 | 등가 (`=`, `IN`) | 파티션 키(`par_b`)로 이미 프루닝됨 |
| WHERE | sort_c | 있음 | 등가 (`=`) | Write Ordering 2순위 |

> **핵심 발견**: `sort_b = par_b`는 동일 컬럼이다. 파티션 키(`par_b`)로 이미 파티션 프루닝이 수행되므로, Write Ordering에서 sort_b를 별도로 정렬하는 것은 **중복 비용**이다.
...
| `par_a` | identity | ✅ 벤치마크 검증 | Cardinality 4 (A, B, C, D). 매우 낮아 identity 최적 |
| `par_b` | identity | ✅ 벤치마크 검증 | par_a별 42~72개, 전체 조합 248개. 상위 50개가 대부분의 데이터 차지. 정기 Compaction 필수 |
...
> **결론**: 3번째 파티션 키로 par_b identity가 현실적인 유일한 선택지이다. par_c, par_d는 identity 사용이 불가능하며, bucket 적용 시에도 조합 수가 par_b보다 줄어들지 않는다. 하위 ~190개 조합의 small file은 **정기 Compaction으로 관리**한다.
...
**TABLE_A 판단**: 파티션 프루닝(`day(ts)` + `par_a` + `par_b`) 후에도 `sort_a`, `sort_c`로 추가 필터링이 필요하며, 반복 조회가 발생한다. → **Write Ordering 필요** ✅
...
### 3.4 TABLE_A 적용: sort_a, sort_c + range 모드 근거

| 설정 | 값 | 근거 수준 | 근거 |
|------|-----|-----------|------|
| `write.distribution-mode` | `range` | 📘 일반적 관행 | 다차원 필터 조회에서 Data Skipping 효과 극대화. 쓰기 비용(9.2GiB shuffle)은 24 executor로 44초에 처리 |
| Write Ordering | `sort_a, sort_c ASC NULLS FIRST` | ✅ 벤치마크 검증 | 조회 패턴 분석 결과: sort_a(빈도 높음) → sort_c(보조). sort_b는 par_b와 동일 컬럼이므로 파티션 프루닝으로 대체, 제외 |

**Write Ordering 컬럼 순서의 중요성**

```sql
WRITE ORDERED BY sort_a, sort_c
```

정렬은 **왼쪽 컬럼부터 우선 적용**된다. 따라서:
- `sort_a` 단독 필터: ✅ Data Skipping 효과 높음
- `sort_a + sort_c` 필터: ✅ 효과 높음
- `sort_c` 단독 필터 (sort_a 없이): ⚠️ 효과 제한적
...
| 기존 | 변경 후 (권장) | 근거 |
|------|-------------|------|
| `sort_a, sort_b, sort_c` | `sort_a, sort_c` | sort_b = par_b(동일 컬럼)이므로 파티션 프루닝으로 이미 처리됨. Write Ordering에 포함하면 정렬 비용만 발생하고 추가 skipping 효과 없음 |
...
**TABLE_A 판단**: sort_a, sort_c 2개 컬럼 체제에서 sort_c 단독 필터 빈도가 아직 충분히 높지 않다. → **현재 Write Ordering 유지, sort_c 단독 필터 빈도 추가 확인 후 결정** 📘 일반적 관행
...
```
일반 정렬 (sort_a, sort_c):        Z-ordering (sort_a, sort_c):
sort_c                               sort_c
  │                                      │
...
→ sort_a 단독 필터: 정렬이 유리      → sort_a, sort_c 개별 필터:
→ sort_c 단독 필터: 효과 없음           Z-ordering이 균등하게 유리
```
...
TABLE_A의 조회 패턴이 **다차원 필터**(ts + par_a/par_b + sort_a/sort_c)인 경우, Z-ordering은 Write Ordering 대비 이점이 있다:

| 조회 패턴 | Write Ordering 효과 | Z-ordering 효과 |
|----------|-------------------|----------------|
| `sort_a = 'x'` (단독) | ✅ 높음 (1순위 정렬 컬럼) | ✅ 높음 |
| `sort_c = 'z'` (단독) | ⚠️ 제한적 (2순위) | ✅ 높음 |
| `sort_a = 'x' AND sort_c = 'z'` | ✅ 높음 | ✅ 높음 |

> **참고**: sort_b(=par_b)는 파티션 키로 이미 프루닝되므로, Write Ordering과 Z-ordering 모두에서 별도 고려 불필요.
...
CALL catalog.system.rewrite_data_files(
  table => 'db.TABLE_A',
  strategy => 'sort',
  sort_order => 'zorder(sort_a, sort_c)',
...
```
1단계 (현재): Write Ordering (sort_a, sort_c)으로 쓰기
...
3단계 (Z-ordering 적용 시): Compaction 대상 컬럼 선정
  → 파티션 키(par_a, par_b)는 이미 프루닝되므로 Z-ordering 대상에서 제외
...
| `sort_c` | **검토 권장** | Write Ordering 2순위. min/max 범위가 넓어 등가 필터 시 Bloom Filter 효과 기대 |
...
| 비즈니스 키 파티션 | `identity`/`bucket` | `identity` (par_a, par_b) | ✅ 벤치마크 검증 | par_a Cardinality 4, par_b 조합 248개. Compaction으로 small file 관리 |
| Write Ordering | 사용/미사용, 컬럼 선택 | `sort_a, sort_c` (사용) | ✅ 벤치마크 검증 | 파티션 프루닝 후 추가 필터 필요. sort_b(=par_b) 제외 |
| Distribution Mode | `none`/`hash`/`range` | `range` | 📘 일반적 관행 | Write Ordering 사용 시 range 필수 |
| Z-ordering | 미적용/Compaction 시 적용 | 미적용 (2단계 검토) | 📘 일반적 관행 | sort_c 단독 필터 빈도 추가 확인 후 결정 |
| Column Metrics | `none`/`truncate`/`full` | 기본값 유지 + array는 `none` | 📘 일반적 관행 | 필터 대상 컬럼 통계 수집 필수 |
| Bloom Filter | 컬럼별 on/off | sort_c 검토 | 📘 일반적 관행 | Write Ordering 2순위 컬럼의 등가 필터 보완 |
...
```sql
-- sort_b(=par_b)는 파티션 프루닝으로 대체되므로 제외
ALTER TABLE catalog.db.TABLE_A
WRITE ORDERED BY
...
```sql
-- Write Ordering 2순위 컬럼에 Bloom Filter 적용
ALTER TABLE catalog.db.TABLE_A
SET TBLPROPERTIES (
    'write.parquet.bloom-filter-enabled.column.sort_c' = 'true'
);
