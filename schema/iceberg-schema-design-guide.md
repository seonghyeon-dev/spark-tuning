...
**다루는 것**
- Iceberg 테이블의 파티션 설계 (Hidden Partitioning, 트랜스폼 선택)
- Write Ordering 및 `write.distribution-mode` 설계
- 버킷팅(Bucketing)과 Z-ordering 적용 검토
- 데이터 타입, Nullable, 테이블 속성 등 스키마 수준 설계
- 각 설계의 TABLE_A 적용 근거

**다루지 않는 것**
- 컴팩션(`rewrite_data_files`) 운영 전략 (별도 문서 예정)
- 스냅샷 관리 및 만료 정책
- Spark Job 리소스/성능 튜닝 (→ `tuning/spark-tuning-guide.md` 참조)
...
| 소파일 위험 기준 | 파티션당 100MB 미만이 다수 발생 |

파티션이 지나치게 세분화되면 파일 수가 급증하여 **소파일 문제**가 발생한다. 메타데이터 오버헤드 증가, 파일 열기/닫기 비용 증가로 읽기·쓰기 성능이 모두 저하된다.
...
> ⚠️ **검증 필요**
> 하루치 데이터 적재 후 `day × par_a × par_b` 조합별 파일 수와 크기 분포를 확인하여 소파일 문제 여부를 판단해야 한다.
...
| 소파일 문제 | 태스크 수만큼 파일 생성 가능 | 파티션별 파일 수 제어 가능 |
...
| 쓰기·읽기 모두 중요 (TABLE_A) | `range` + ordering + 컴팩션 | 쓰기 시 기본 정렬 + 컴팩션 시 소파일 병합·재정렬 |
...
### 3.4 TABLE_A 적용: sort_a, d, e + range 모드 근거

| 설정 | 값 | 근거 수준 | 근거 |
|------|-----|-----------|------|
| `write.distribution-mode` | `range` | 📘 일반적 관행 | 다차원 필터 조회에서 data skipping 효과 극대화. 쓰기 비용(9.2GiB shuffle)은 24 executor로 44초에 처리 |
| Write Ordering | `sort_a, d, e ASC NULLS FIRST` | ⚠️ 개선 필요 | 조회 패턴의 필터 컬럼 기준. 실제 조회 빈도 데이터로 재검증 필요 |
...
| `day × 파티션 키` 조합별 데이터가 100MB 미만으로 소파일 문제가 있는가? | **Bucket으로 파티션 수 축소** | identity 유지 |
...
| 카디널리티 높음 (수백~수천) | ❌ 파티션 폭증, 소파일 | ✅ N개로 제어 가능 |
...
**TABLE_A 판단**: 조회 패턴이 다차원 필터이며 sort_b/e 단독 필터 빈도가 불명확. → **조회 로그 확보 후 결정** ⚠️
...
TABLE_A의 조회 패턴이 **다차원 필터**(ts + par_a/b + sort_a/d/e)인 경우, Z-ordering은 Write Ordering 대비 이점이 있다:
...
| **컴팩션** | Iceberg `rewrite_data_files` 프로시저. 소파일 병합, 정렬(Z-ordering 포함) 적용 |
| **소파일 문제** | 파티션이 과다하거나 배치 빈도가 높아 작은 파일이 다수 생성되는 현상. 읽기 성능 저하의 주요 원인 |
