...
>
> ⚠️ **2026-09-05 이전 자료는 이름이 다르다.** 당시 `col_a`는 `par_b`, `col_b`는 `sort_c`였다. `sort_b`는 이름이 바뀌지 않았지만, 예전 가이드는 역할을 뒤바꿔 적어 **`sort_c`(현 `col_b`)를 Sort Order 2순위로, `sort_b`를 일반 컬럼으로** 기술했다. 예전 캡처·회의 자료의 컬럼명과 역할 설명을 그대로 옮기지 말 것.
