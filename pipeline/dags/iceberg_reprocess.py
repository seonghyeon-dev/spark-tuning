...
grep "TODO(연결)" 으로 전체 확인.
  ...
  8. Compaction / 자기 재trigger — trigger 호출 (conf 날짜/시간 형식 확인)
────────────────────────────────────────────────────────────────────
"""
...
def dag():  # 함수명 dag() 고정 — DAG 정체성은 파일명(dag_id)이 담당
    ...
    def trigger_compaction(ti=None):
        ...
        TODO(연결): trigger 호출부 + 날짜/시간 형식(기존 Compaction UI params와 일치)."""
        metas = _collect_metas(ti)
        daily_targets: dict[str, list[str]] = {}
        ...
        for target_dt, tables in daily_targets.items():
            trigger_dag_run(DAILY_COMPACTION_DAG_ID,  # noqa: F821  TODO(연결)
                            conf={"target_dt": target_dt, "tables": tables})
        if hourly_tables:
            trigger_dag_run(HOURLY_COMPACTION_DAG_ID,  # noqa: F821  TODO(연결)
    ...
    def check_loop(cfg: dict, ti=None):
        ...
        실패 테이블 제외 — 같은 밤에 깨진 데이터 반복 재시도 안 함 (다음날 회수)."""
        if not cfg:
            return False
        if not [m for m in _collect_metas(ti) if m["leftover"]]:
            return False
    ...
    def retrigger_self(cfg: dict):
        """자기 재trigger (설계 5.5) — 첫 회차가 확정한 조회 범위·tables를 conf로 승계."""
        trigger_dag_run(DAG_ID,  # noqa: F821  TODO(연결)
    ...
    chain(
        run_cfg,
        *[build_table_group(t, run_cfg) for t in ALL_TABLES],  # 테이블 순차
        trigger_compaction(),
        check_loop(run_cfg),
        retrigger_self(run_cfg),
    )
