...
@dag(
    ...
    params={
        ...
        # 수동: 대상 날짜. 그저께 이전만 허용 — prepare_run에서 검증. 기존 DAG과 동일한 달력 UI
        "target_dt": Param(None, type=["null", "string"], format="date"),
        # 수동: ts 범위 축소. 기존 DAG과 동일한 date-time 형식 → prepare_run에서 ts 문자열로 변환
...
def dag():  # 조직 컨벤션: 함수명 dag() 고정 — DAG 정체성은 파일명(dag_id)이 담당
    ...
    def prepare_run(params=None, dag_run=None) -> dict:
        ...
        기존 append DAG의 get_time 패턴과 동일. 형식 오류·잘못된 날짜는 여기서
        ...
        if params.get("target_dt"):
            ...
            d = pendulum.parse(params["target_dt"], tz=KST)
            if d >= base.subtract(days=2):
                raise ValueError(
                    "target_dt는 그저께 이전 날짜만 허용 — 전날/당일은 append 조회 범위와 겹침"
                )
            ts_from, ts_to = ts_str(d), ts_str(d.add(days=1))
            wait_bound = ts_to  # 그저께 이전은 append 범위 밖
        else:
        ...
        # 수동 ts 범위 축소 — 기본 경계 안쪽으로만 좁힌다 (경계 밖 확장 불가)
        if params.get("start_time"):
            ts_from = max(ts_from, ts_str(pendulum.parse(params["start_time"], tz=KST)))
        if params.get("end_time"):
            ts_to = min(ts_to, ts_str(pendulum.parse(params["end_time"], tz=KST)))
    ...
    def _collect_metas(dag_run, ti) -> list[dict]:
        """Spark가 성공한 테이블의 meta만 수집."""
        metas = []
        for t in ALL_TABLES:
            task_instance = dag_run.get_task_instance(f"reprocess_{t.get_name()}.append_data")
            if task_instance and task_instance.state == "success":
                meta = ti.xcom_pull(
                    task_ids=f"reprocess_{t.get_name()}.get_table_jobs", key="meta"
                )
                if meta:
                    metas.append(meta)
        return metas
    ...
    def trigger_compaction(dag_run=None, ti=None):
        ...
        """
        metas = _collect_metas(dag_run, ti)
    ...
    def check_loop(cfg: dict, dag_run=None, ti=None):
        ...
        pending = [m for m in _collect_metas(dag_run, ti) if m["leftover"]]
        if not pending:
            return False
    ...
    retrigger = TriggerDagRunOperator(
        ...
        conf={
            # 경계 기준일·회차는 prepare_run의 정규화 값에서 단일 소스로 가져온다 (설계 5.5)
            "loop_count": "{{ (ti.xcom_pull(task_ids='prepare_run')['loop_count'] | int) + 1 }}",
            "base_date": "{{ ti.xcom_pull(task_ids='prepare_run')['base_date'] }}",
        },
    )
    ...
    prev >> trigger_compaction() >> check_loop(run_cfg) >> retrigger
