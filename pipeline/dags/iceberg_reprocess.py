...
── 기존 구현 연결 지점 (grep "TODO(연결)") ────────────────────────
  ...
  2. ConvertFileTaskGroup import + 주입 계약 (build_table_group 주석 참조)
...
def _select(sql: str, binds: dict) -> list[dict]:
    """SELECT → dict row 목록 (OracleHook, append DAG과 동일 conn)."""
...
def reprocess_get_jobs(cfg: dict, *, table_name: str, group: str, run_id, ti) -> bool:
    ...
    committed = {
        ...
        if snapshot_exists(table_name, b)
    }
    if committed:
        _update_status(
...
def dag():  # 함수명 dag() 고정 — DAG 정체성은 파일명(dag_id)이 담당
    ...
    def build_table_group(table, run_cfg):
        """기존 ConvertFileTaskGroup 재사용 (설계 5.2) — 재처리 조회 로직만 주입.

        TODO(연결): ConvertFileTaskGroup 실제 생성자에 맞출 것. 템플릿은 자기 그룹
        컨텍스트에서 아래 계약대로 동작해야 한다:
          - get_jobs_fn(cfg)를 @task.short_circuit(trigger_rule="all_done",
            ignore_downstream_trigger_rules=False)로 래핑해 task_id="get_jobs"로 생성
            (앞 테이블 실패에도 실행 + skip을 그룹 내로 한정)
          - Spark task는 meta.num_executors 사용 + batch_id를 snapshot property로 커밋,
            retry 시 자기 batch_id snapshot 확인 후 즉시 성공 종료 (중복 방어, 설계 4.2)
          - update_success/update_failure는 meta.job_ids로 DONE/FAILED 처리
        """
