...
def dag():  # 조직 컨벤션: 함수명 dag() 고정 — DAG 정체성은 파일명(dag_id)이 담당
    ...
    def prepare_run(params=None, dag_run=None) -> dict:
        ...
        if params.get("target_dt"):
            # 수동: 지정 날짜 전체 (WAIT+FAILED). 그저께까지의 과거만 허용 (설계 5.1)
            # 전날/당일만 거부 — 그저께(D-2)는 append 조회 범위를 완전히 벗어나 있어 안전
            d = pendulum.parse(params["target_dt"], tz=KST)
            if d >= base.subtract(days=1):
                raise ValueError(
                    "target_dt는 그저께까지의 과거 날짜만 허용 — 전날/당일은 append 조회 범위와 겹침"
                )
        ...
        if params.get("start_time"):
            st = pendulum.parse(params["start_time"], tz=KST).in_timezone(KST)
            ts_from = max(ts_from, ts_str(st))
        if params.get("end_time"):
            et = pendulum.parse(params["end_time"], tz=KST).in_timezone(KST)
            ts_to = min(ts_to, ts_str(et))
