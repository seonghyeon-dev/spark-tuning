...
@dag(
    ...
    params={
        ...
        # 수동: 대상 날짜. 그저께 이전만 허용 — task에서 검증. 기존 DAG과 동일한 달력 UI
        "target_dt": Param(None, type=["null", "string"], format="date"),
        # 수동: ts 범위 축소. 기존 DAG과 동일한 date-time 형식 → 내부에서 ts 문자열로 변환
...
def dag():  # 조직 컨벤션: 함수명 dag() 고정 — DAG 정체성은 파일명(dag_id)이 담당
        ...
        with TaskGroup(group_id=f"reprocess_{tbl}") as group:
            ...
            def get_table_jobs(**context):
                ...
                if params.get("target_dt"):
                    ...
                    d = pendulum.parse(params["target_dt"], tz=KST)  # date 형식 param
                ...
                # 수동 ts 범위 축소 — date-time param을 ts 문자열로 변환
                s = (ts_str(pendulum.parse(params["start_time"], tz=KST))
                     if params.get("start_time") else d2_start)
                e = (ts_str(pendulum.parse(params["end_time"], tz=KST))
                     if params.get("end_time") else d1_end)
    ...
    def _collect_metas(context) -> list[dict]:
        ...
        metas = []
        for t in ALL_TABLES:
            ti = dag_run.get_task_instance(f"reprocess_{t.get_name()}.append_data")
    ...
    retrigger = TriggerDagRunOperator(
        ...
        conf={
            ...
            # 주의: ds(logical date)는 01:00 스케줄 run에서 전날 날짜이므로 사용 금지.
            #       실행 시각 기준인 data_interval_end의 날짜를 써야 "오늘"이 된다
            "base_date": "{{ dag_run.conf.get('base_date', data_interval_end | ds) }}",
        },
    )
