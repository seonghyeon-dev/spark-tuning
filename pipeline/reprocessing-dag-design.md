...
| Compaction DAG | **hourly DAG 1개**(`35 * * * *` → `45 * * * *`, 직전 1시간치) + **daily DAG 1개**(현재 `35 0 * * *` → `0 1 * * *`, 전일치). 각 DAG 내부에서 소속 테이블 task가 순차 실행. `max_active_runs=1`. UI 수동 실행용 params: daily는 `target_dt`, hourly는 `start_time`/`end_time` |
...
- 잔류 데이터 회수는 **재처리 DAG 1개**(1일 주기, 04:00 KST)가 전담한다. 테이블별 task로 구성 (Compaction DAG과 동일 패턴)
...
└──────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────┐
│ 재처리 DAG 1개 (1일 주기, 04:00 KST) — 잔류 데이터 회수 전담    │
...
**핵심 규칙**: append는 항상 "실행 시각-24시간 이후"만 조회한다. 재처리의 WAIT_SCHEDULING 조회 상한을 **전날 04:00**(= 재처리 스케줄 시각 `RUN_HOUR` - 24시간)으로 잡으면, 재처리가 04:00 이후에 도는 한 append의 조회 하한은 항상 전날 04:00 이상이므로 **두 범위는 절대 겹치지 않는다.** FAILED는 append가 아예 조회하지 않으므로 겹침 걱정 없이 전날·그저께 전체를 잡는다.

**오늘 = 4일** (전날 = 3일, 그저께 = 2일)로 두고, 오늘 4일 04:00 실행 기준(`RUN_HOUR=4`) (`ts`는 `YYYYMMDDHHmmSSsss` 문자열 비교):

| 대상 | ts 범위 | 이유 |
|------|---------|------|
| FAILURE | `20260702000000000` ≤ ts < `20260704000000000` (그저께 00:00 ~ 전날 끝) | append는 FAILED를 조회하지 않으므로 전 구간 안전 |
| WAIT_SCHEDULING | `20260702000000000` ≤ ts < `20260703040000000` (그저께 00:00 ~ 전날 04:00) | 전날 04:00 이후는 append가 아직 조회 가능한 영역 |
| 그저께 이전 | 조회 안 함 | **알림 → 수동 처리** (섹션 8) |

> **경계값은 실행 시각이 아니라 그 날의 `RUN_HOUR`(04:00) 고정값으로 계산한다.** 재실행이나 loop 회차가 늦게 돌아도 경계가 append 조회 하한보다 항상 과거이므로 안전이 유지된다.
>
...
```
[FAILURE] 전날(3일) 15:00 배치 실패 → 오늘(4일) 04:00 재처리가 회수 (최대 하루 지연)

[WAIT_SCHEDULING]   전날(3일) 08:00 생성 후 계속 미처리
         4일 04:00  재처리: WAIT_SCHEDULING 상한이 3일 04:00 → 대상 아님 (아직 append 담당)
         4일 08:00  생성 24시간 경과 → append 조회 범위 이탈
         다음날(5일) 04:00  재처리: WAIT_SCHEDULING 범위 [3일 00:00 ~ 4일 04:00) → 회수 ✅ (최대 약 이틀 지연)
```
...
**같은 테이블에 동시에 append 커밋**을 시도하는 상황은 실제로 발생한다 (재처리는 04:00에
...
| 전날 WAIT_SCHEDULING 전체를 재처리가 가져가기 (UPDATE 선점 또는 Airflow pool로 경합 제어) | UPDATE 선점 후 재조회는 stat_desc(CLOB) 조건이 필요해 성능 문제. pool은 테이블 20개+ 구조에서 테이블별 pool 난립 또는 전역 병목. 조회 범위를 겹치지 않게 하는 것이 가장 단순 (전날 04:00 이후 WAIT는 다음날 회수 — 하루 지연 허용) |
...
- **배치 레벨 재시도**: task 재시도 소진 후 FAILURE 확정 → 다음날 04:00 재처리 DAG이 재시도. 별도 retry 카운트는 DB에 저장하지 않음 (재시도는 task가, 배치 재시도는 상태 재조회가 담당)
...
오늘(4일) 04:00  재처리 DAG이 FAILED를 재적재 → 중복!
```
    ...
    SELECT k_1, k_2, k_3, ts, base_path, param,
     ...
     WHERE table_name = :tbl
       ...
       AND ( status = 'FAILURE'                                   -- FAILURE: 전 구간
             OR (status = 'WAIT_SCHEDULING' AND ts < :wait_bound)           -- WAIT_SCHEDULING: 전날 04:00 이전만
           )
...
| 작업 | duration |
|------|----------|
| hourly Compaction | 10~12분 |
| daily Compaction | 30~60분 |
| expire snapshots | 6~12분 |
| remove orphan files | 5~9분 |
| rewrite manifests | 2~3분 |

daily Compaction 외에는 전부 12분 이내라 한 시간 슬롯에 여유 있게 들어간다.

| cron | 작업 | 변경 | 종료(최악) | 다음 hourly까지 |
|------|------|------|-----------|----------------|
| `45 * * * *` | hourly Compaction | `35 * * * *` → **:45** | `:57` | 정각까지 3분 |
| `0 1 * * *` | daily Compaction | `35 0 * * *` → **01:00** | 02:00 | 2시간 슬롯 |
| `0 3 * * *` | expire snapshots | 01:05 → **03:00** | 03:12 | 33분 |
| `0 4 * * *` | **재처리 DAG** | 01:00 → **04:00** | 가변 | |
| `0 5 * * *` | remove orphan files | 01:35 → **05:00** | 05:09 | 36분 |
| `0 6 */3 * *` | rewrite manifests | 02:05 → **06:00** (3일마다) | 06:03 | 42분 |

전부 정각 시작이고 매시 `:45~:57`의 hourly 창을 아무도 건드리지 않는다.
...
순서(Compaction → expire → 재처리 → orphan → manifests)는 Iceberg 권장 순서를 유지한 것이다. 다만 **remove orphan files를 재처리 뒤에 둘 필연적 이유는 없다** — `older_than`이 3일이면 그날 만든 파일은 애초에 삭제 대상이 아니다.

**전제와 한계**

| 항목 | 내용 |
|------|------|
| 재처리 적재분 Compaction | daily 정기 run(01:00)이 재처리(04:00)보다 **먼저** 끝나므로 재처리 적재분은 정기 run이 덮지 못한다. 6.3의 "조건 없이 적재분 전부 trigger"가 이를 담당한다 — 조건부 생략을 두지 않은 결정이 여기서 값을 한다 |
...
| hourly와의 겹침 | hourly Compaction이 매시간 도는 이상 60분짜리 daily 작업과의 겹침은 피할 수 없다. daily Compaction과는 대상 테이블 그룹이 달라 Iceberg 충돌이 없고 리소스 경합만 있다. **expire/orphan/manifests는 전체 테이블 대상이라 hourly와 같은 테이블을 건드리므로**, 정각에 시작해 `:45`까지 45분을 확보한다. 세 작업 모두 12분 이내라 충분하다 |
...
| hourly Compaction DAG | 스케줄 + params + task 생성 방식 | `35 * * * *` → `45 * * * *` (6.2). 나머지는 daily와 동일. `target_dt` 대신 `start_time`/`end_time`을 담는다 |
