...
| param | 기본값 | 설명 |
|-------|--------|------|
| `tables` | 전체 테이블 | 처리 대상 테이블 multi-select — 1개/여러 개/전체 선택 가능. 정기 실행은 기본값(전체). 선택지·기본값은 append DAG과 동일한 **`IcebergTable` Enum**에서 생성 (`Param(type="array", items={"enum": [...]})`) — 테이블 추가/제거 시 Enum 한 곳만 수정하면 append/재처리가 함께 반영되는 단일 소스 |
...
**Compaction DAG params** (양쪽 공통으로 `tables` multi-select 추가 — 기본 전체, 수동/trigger 시 일부 선택. 선택지는 재처리 DAG과 동일하게 `IcebergTable` Enum에서 생성 권장):
...
| 5 | loop 재trigger 조건 = 잔여분 존재 AND 해당 테이블 Spark 성공. 경계 기준일은 conf `base_date`로 고정 전달 | 5.5 |
| 6 | 수동 `target_dt`는 그저께 이전만 허용 — task에서 검증 후 거부 | 5.1 |
