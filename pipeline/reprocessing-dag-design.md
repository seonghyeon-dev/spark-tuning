...
| `target_dt` | 없음 | 지정 시 해당 날짜의 WAIT+FAILED 전체를 대상으로. **그저께까지의 과거 날짜만 허용** — 전날/당일은 WAIT 조회가 append 조회 범위와 겹치므로 prepare_run이 검증 후 거부한다 (그저께는 append 범위를 완전히 벗어나 안전) |
...
2. Airflow UI에서 재처리 DAG을 params 지정 후 수동 실행:
   ...
   - `target_dt`: 대상 날짜 — **그저께까지의 과거만 허용** (전날/당일은 append 조회 범위와 겹쳐 거부. 전날 잔여분은 다음날 정기 실행이 자동 회수)
...
| 6 | 수동 `target_dt`는 그저께까지의 과거만 허용 (전날/당일 거부) — prepare_run에서 검증 | 5.1 |
