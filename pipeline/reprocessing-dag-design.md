...
append DAG(약 5분 주기)의 `get_jobs` task는 Oracle Job History 테이블에서 처리 대상을 조회한다. Job History 테이블은 **날짜 키로 파티셔닝**되어 있어 Partition Pruning을 위해 조회 기간을 **최근 1일**로 제한한다. 조회 기간을 늘리면 조회 성능이 크게 저하되므로 이 제약은 유지해야 한다.
...
```
┌─────────────────────────────────────────────────────────────┐
│ append DAG (약 5분 주기) — 신규 데이터 전담                    │
│   조회: 최근 1일 파티션, status = 'WAIT'                      │
│   재시도 없음. 실패 건은 FAILED 확정 후 재처리 DAG에 위임        │
└─────────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────┐
│ 재처리 DAG (1일 주기, 01:00 KST) — 잔류 데이터 회수 전담        │
│   조회: D-1 파티션의 FAILED + D-2 파티션의 WAIT/FAILED         │
│   상한: row 1,000 / 총 크기 16GB                              │
│   처리 후 기존 Compaction DAG trigger                         │
└─────────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────┐
│ 수동 처리 (매일 모니터링) — 비정상 상황 전담                    │
│   D-3 이상 잔류·상한 초과 이월 건: 알림 수신 → 원인 확인 →      │
│   UI에서 재처리 DAG을 target_dt(+시간 범위) 지정 수동 실행      │
└─────────────────────────────────────────────────────────────┘
```
...
**전제**: append DAG의 최근 1일 조회는 "실행 시각부터 24시간 전까지"이다(Job History 시간값 기준). 14일 10시에 실행되면 13일 10시~14일 10시를 조회한다. 즉 어제 파티션의 데이터는 오늘 중까지도 계속 append DAG의 조회 대상이다.

재처리 DAG이 도는 14일 01:00 기준으로 파티션별 담당을 정하면:
...
| 14일 (오늘) | — | 조회 안 함 | 오늘 데이터의 WAIT는 append 담당. 오늘 발생할 FAILED는 내일 D-1로 회수됨 |
| 11일 이전 (D-3~) | WAIT/FAILED | 수동 | **알림 → 수동 처리** (섹션 8) |

정리하면 재처리 DAG의 조회 대상은 **어제 파티션의 FAILED + 그저께 파티션의 WAIT/FAILED** 두 가지다.

> **왜 00:00이 아니라 01:00에 도는가**: Job History의 시간값이 13일 23:59인 데이터가 반드시 13일 중에 테이블에 쌓인다는 보장이 없다 — 자정을 조금 넘겨 적재될 수 있다. D-1 파티션이 안정된 후 조회하도록 1시간 버퍼를 둔다.
>
> **01:00 실행이 만드는 1시간 틈**: 14일 01:00에 append의 조회 범위는 13일 01:00~14일 01:00이다. 따라서 13일 00:00~01:00 생성분 WAIT는 이 시점에 append 범위에서 이미 이탈했지만 D-1이므로 재처리도 집지 않는다. 이 구간은 15일 01:00에 D-2로 회수된다 — 하루 지연될 뿐 영구 누락은 아니다.
>
...
```
13일 08:00   생성. append DAG이 약 5분 주기로 조회 (24시간 동안)
14일 01:00   재처리 실행 → 13일 WAIT는 아직 append 담당이므로 제외
14일 08:00   생성 후 24시간 경과 → append 조회 범위에서 이탈
15일 01:00   재처리 실행 → 13일은 이제 그저께(D-2) → WAIT 회수 ✅
```
...
- **배치 레벨 재시도**: task 재시도 소진 후 FAILED 확정 → 다음날 01:00 재처리 DAG이 재시도. 별도 retry 카운트는 DB에 저장하지 않음
...
| 항목 | 값 | 근거 |
|------|-----|------|
| schedule | `0 1 * * *` (KST) | Job History 시간값이 어제 23:59인 데이터가 자정을 넘겨 적재될 수 있어, D-1 파티션이 안정된 후 조회하도록 1시간 버퍼. 일일 Compaction(00:35) 이후 실행이므로 적재분 Compaction은 전부 trigger로 처리 (섹션 6.3) |
...
| params | `target_dt`(날짜), `start_time`/`end_time`(시간 범위, 선택) | UI 수동 실행: 특정 날짜 지정, 필요 시 Job History 시간값 범위로 대상 축소 (섹션 8.3) |
...
1. **대상 조회** — 날짜 파티션별 등호 조회로 Partition Pruning 유지 (범위 조회 금지), **row 수 상한(1,000) 적용**:
   - D-1 파티션: `status = 'FAILED'`
   - D-2 파티션: `status IN ('WAIT', 'FAILED')`
   - 수동 실행(`target_dt` 지정) 시: 해당 파티션의 `WAIT` + `FAILED`. `start_time`/`end_time` 지정 시 Job History 시간값 범위 조건 추가
...
한 번의 실행에서 처리하는 물량에 **이중 상한**을 둔다. 두 상한은 보호 대상이 다르다.

| 항목 | 값 | 보호 대상 | 근거 수준 |
|------|-----|----------|----------|
| 1회 조회 row 수 상한 | 1,000 (ROWNUM) | Oracle SELECT 성능, XCom 크기, avro 경로 목록 파일 크기 | ⚠️ 러프 설정 — 재검증 필요 |
| 1회 처리 총 크기 상한 | 16GB | Spark 리소스, 처리 소요시간. 벤치마크 검증 범위(~8GB, num-executors 24)의 2배 이내 | ⚠️ 러프 설정 — 재검증 필요 |
| num_executors 상한 | 24 | 벤치마크에서 32 이상은 성능 저하 확인 (spark-tuning-guide.md 2.2.3) | ✅ 벤치마크 검증 |

**row 상한의 규모 감각**: append DAG은 현재 약 5분 주기이고 5분치 ≈ 200 rows(Job History 시간값과 현재 시간이 동기화된 정상 상태 기준). 즉 1,000 rows ≈ 약 25분치 물량이다. 정상 운영에서 하루 잔여분은 이보다 훨씬 적을 것으로 예상하지만, 상한값 자체는 검증된 값이 아니므로 운영 데이터로 재조정한다.

> **상한 초과 시 알림이 필수인 이유**: 상한에 잘린 D-2 WAIT는 다음날 D-3이 되어 **자동 재처리 범위를 벗어난다.** 초과분 이월이 발생하면 반드시 알림을 발송해 수동 처리(섹션 8.3)로 연결해야 한다. 하루치 전체가 밀리는 수준의 대량 잔류는 애초에 자동 회수 대상이 아니라 원인 파악이 필요한 장애 상황이다.

> **K8S 리소스 경합 주의**: 재처리 Spark job(최대 24 executor, 96 core, ~213GB)이 도는 동안에도 약 5분 주기 append job(동일 규모)이 뜬다. 동시 실행 시 최대 **~192 core, ~427GB**가 필요하다. 클러스터 여유가 부족하면 재처리 job의 executor 상한을 낮춘다(예: 12 — 지연 데이터이므로 처리 속도의 우선순위가 낮음).
...
- **적재한 모든 날짜(D-1 포함)를 trigger**: 재처리 DAG(01:00)은 일일 Compaction(00:35) **이후**에 실행되므로, 재처리 적재분은 스케줄된 Compaction run이 커버하지 못한다. 적재한 날짜는 전부 trigger해야 한다. 같은 날짜를 두 번 Compaction해도 두 번째는 합칠 파일이 없어 사실상 no-op이라 무해
...
| 상한 초과 이월 | 재처리 대상이 row 1,000 또는 총 크기 16GB 초과 | append DAG 장애 여부 확인 → 수동 판단. **이월된 D-2 WAIT는 다음날 자동 범위(D-3)를 벗어나므로 방치 금지** |
...
2. Airflow UI에서 재처리 DAG을 params 지정 후 수동 실행 (Compaction DAG 수동 실행과 동일한 패턴):
   - `target_dt`: 대상 날짜 (필수)
   - `start_time` / `end_time`: Job History 시간값 범위 (선택) — 잔류량이 상한(row 1,000 / 16GB)을 넘는 날은 시간 범위로 쪼개서 여러 번 실행
...
| 8 | 처리 상한 재검증 | row 1,000 / 총 크기 16GB은 러프 설정 — 운영 데이터(잔류량 분포, SELECT/XCom/Spark 소요시간)로 재조정 |
...
ROW_LIMIT = 1000                 # 1회 조회 row 수 상한 (⚠️ 러프 설정 — 재검증 필요)
SIZE_LIMIT_MB = 16 * 1024        # 1회 처리 총 크기 상한 16GB (⚠️ 러프 설정 — 재검증 필요)
...
@dag(
    ...
    schedule="0 1 * * *",         # 01:00 KST — D-1 파티션 안정화 버퍼 (섹션 5.1)
    start_date=pendulum.datetime(2026, 7, 1, tz=KST),
    catchup=False,
    max_active_runs=1,
    params={                      # UI 수동 실행용 (섹션 8.3)
        "target_dt": None,        # 대상 날짜 (YYYYMMDD)
        "start_time": None,       # Job History 시간값 범위 시작 (선택)
        "end_time": None,         # Job History 시간값 범위 끝 (선택)
    },
)
def iceberg_reprocess():
    ...
    def get_reprocess_jobs(**context):
        ...
        target_dt = params.get("target_dt")
        start_time, end_time = params.get("start_time"), params.get("end_time")
        today = pendulum.now(KST).date()

        # 1) 대상 조회 — 날짜 파티션 등호 조회 (Partition Pruning 유지) + row 수 상한
        if target_dt:
            # 수동 실행: 지정 날짜의 WAIT + FAILED (+선택적 시간값 범위)
                ...
                SELECT * FROM (
                    SELECT job_id, status, stat_desc, avro_path, file_size_mb
                     ...
                     WHERE part_dt = :part_dt
                       ...
                       AND (:start_time IS NULL OR hist_time >= :start_time)   -- 수동 실행 시 시간 범위
                       AND (:end_time   IS NULL OR hist_time <  :end_time)
                     ORDER BY hist_time
                ) WHERE ROWNUM <= :row_limit
            """, part_dt=part_dt, statuses=statuses,
                 start_time=start_time, end_time=end_time,
                 row_limit=ROW_LIMIT - len(jobs))
            if len(jobs) >= ROW_LIMIT:
                # 이월된 D-2 WAIT는 다음날 자동 범위를 벗어나므로 알림 필수 (섹션 5.4)
                send_alert("재처리 row 상한(1,000) 도달 — 초과분 이월, 수동 처리 필요")
                break
