...
> **크기 기준 상한을 두지 않는 이유**: 처리량 통제 수단이 둘이면 둘 사이가 어긋난다. `ROW_LIMIT` 1,000 × DB 2개는 약 40GB로 추정되는데(아래 환산), 여기에 16GB 같은 크기 상한을 겹치면 크기 쪽이 항상 먼저 걸려 `ROW_LIMIT` 값이 무의미해진다. 상한을 하나로 두고 그 값을 실측으로 조정하는 편이 낫다.
>
> | 근거 | 값 |
> |------|-----|
...
> | ⇒ row 1건 | ≈ 20MB |
> | ⇒ ROW_LIMIT 1,000 × DB 2개 = 2,000 rows | ≈ **40GB** (벤치마크 범위의 5배) |
>
...
| 8 | 처리 상한 재검증 | 테이블당·DB당 row 1,000 / loop 10회는 러프 설정 — 운영 데이터로 재조정. row 1,000×DB 2개는 약 40GB로 추정되며(벤치마크 10분치 ≈ 8GB, 5분치 ≈ 200 rows 기준 row당 ≈ 20MB) 벤치마크 범위의 5배다. 백로그가 크게 쌓인 날의 Spark job 크기를 실측해 ROW_LIMIT을 조정한다 |
