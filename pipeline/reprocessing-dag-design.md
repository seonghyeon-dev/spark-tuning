...
│   조회: D-0~D-1 파티션의 FAILED + D-2 파티션의 WAIT/FAILED     │
...
두 DAG이 같은 row를 동시에 집는 경우를 상태·날짜 조건으로 원천 차단한다.

**D-N 표기 기준**: D-N은 Job History의 **파티션 키 날짜**를 가리키며, 기준일은 재처리 DAG 실행 시각(00:00 KST)의 날짜다. 예) 14일 00:00 실행 시 D-0 = part_dt 14일, D-1 = part_dt 13일, D-2 = part_dt 12일.

| 파티션 날짜 | append DAG | 재처리 DAG |
|------------|-----------|-----------|
| D-0 | `WAIT` | `FAILED`만 |
| D-1 | `WAIT` (최근 1일 조회 기간에 포함) | `FAILED`만 |
| D-2 | 조회 안 함 | `WAIT` + `FAILED` |
| D-3 이상 | 조회 안 함 | 조회 안 함 → **알림 → 수동 처리** |

> **왜 D-1의 WAIT는 재처리 대상이 아닌가**: append DAG의 최근 1일 조회는 실행 시각 기준 rolling 방식이다(14일 10시 실행 → 13일 10시~14일 10시 조회). 재처리 DAG이 도는 **00:00 정각에는 이 rolling 범위(13일 00:00~14일 00:00)가 D-1 파티션 전체와 정확히 일치**한다. 즉 D-1의 WAIT는 아직 append DAG이 조회 가능한 영역이므로, 재처리가 집으면 같은 row를 두 DAG이 잡는 경합이 생긴다. 반면 D-2는 rolling 범위를 완전히 벗어나 있어 재처리 담당이다.
>
> **D-0 FAILED를 포함하는 이유**: 00:00 정각 실행이면 D-0 파티션은 사실상 비어 있다(빈 조회 1회). 재처리 DAG 실행이 지연된 경우(재시도 등으로 오전에 실행) 당일 새벽 실패분까지 집기 위한 보험이다.
>
> **경합 방어선**: 만약의 경합에 대비해 IN_PROGRESS 전환 UPDATE는 반드시 `WHERE status = 'WAIT'`(또는 `'FAILED'`) 조건을 포함한 원자적 전환으로 수행한다. 두 DAG이 동시에 시도해도 한쪽만 성공한다.

**WAIT 회수 타임라인 예시** — 13일 08:00에 생성된 WAIT row(part_dt=13일)가 계속 처리되지 못하는 경우:

```
13일 08:00   생성 → append DAG이 매 10분 조회 대상
14일 00:00   재처리 실행: 13일은 D-1 → WAIT 제외 (append가 14일 08:00까지 아직 조회 가능)
14일 08:00   append rolling 조회 범위에서 이탈 → 이후 아무 DAG도 조회하지 않음
15일 00:00   재처리 실행: 13일이 D-2가 됨 → WAIT 회수 ✅
```

append 조회 이탈과 재처리 회수 사이에 공백 구간은 있지만 영구 누락 경로는 없으며, WAIT 생성부터 재처리 회수까지 최대 지연은 약 2일이다. 이것이 재처리 lookback을 2일로 잡은 이유다.
...
| FAILED | update_failure | 재처리 DAG (D-0~D-2) / 수동 (D-3 이상) |
