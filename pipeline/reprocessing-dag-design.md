...
4. **크기 상한 적용** — 병합된 전체 목록에서 앞(가장 오래된 것)부터 누적 16GB를 넘기 직전까지만 담는다. 잘린 뒤쪽은 상태를 건드리지 않고 이월 (loop 회차 또는 다음날 회수)
   - **조회 결과는 있는데 담긴 게 0건이면 알림** (조회 결과 자체가 없으면 처리할 게 없다는 뜻이므로 무음 skip)

> **담긴 게 0건이 되는 경우는 하나뿐이다**: 맨 앞(가장 오래된) 대상 1건이 혼자서 상한보다 크면, 첫 검사에서 바로 멈춰 아무것도 담기지 않는다.
>
> ```
> 대상을 오래된 순으로 세운 뒤 앞에서부터 담는다 (상한 16GB)
>   09:00  20GB  ← 혼자 상한 초과 → 즉시 멈춤, 담김 0건
>   10:00   1GB  ← 담아볼 기회조차 없음
>   11:00   2GB  ←     "
> ```
>
> 문제는 이 상태가 **저절로 풀리지 않는다**는 점이다. 정렬 순서가 항상 오래된 순이므로 그 20GB 건은 다음 실행에도 계속 맨 앞에 서고, 뒤에 줄 선 정상 대상까지 무기한 막는다. 알림에 해당 건의 `ts`와 실제 크기를 담아 사람이 바로 판단할 수 있게 한다 (배치 분할 또는 상한 조정).
...
| 크기 상한에 막힘 | 조회 결과는 있는데 담긴 게 0건 — 가장 오래된 대상 1건이 혼자 상한 초과 (섹션 5.3). 알림에 해당 `ts`와 크기 포함 | 방치하면 그 테이블의 재처리가 계속 막힌다 → 해당 배치를 분할하거나 상한 조정 |
