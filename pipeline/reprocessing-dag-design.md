...
run N: 테이블별 최대 1,000건 처리
       → 잔여분이 있는 테이블 중 이번 회차 Spark가 성공한 테이블이 하나라도 있으면
         check_loop가 자기 자신을 trigger (conf에 loop_count, base_date 전달)
...
- **경계 기준일은 conf로 고정 전달** (`base_date`): 회차가 자정을 넘겨 실행되어도 ts 경계(그저께 00:00 / 전날 01:00)가 첫 회차 기준으로 유지된다
...
2. Airflow UI에서 재처리 DAG을 params 지정 후 수동 실행:
   ...
   - `target_dt`: 대상 날짜 — **그저께 이전만 허용** (전날/당일은 append 조회 범위와 겹쳐 task가 거부. 전날 잔여분은 다음날 정기 실행이 자동 회수)
...
def iceberg_reprocess():
    ...
    def build_table_group(tbl: str) -> TaskGroup:
        with TaskGroup(group_id=f"reprocess_{tbl}") as group:
            ...
            @task.short_circuit(task_id="get_table_jobs",
                                ...
                                ignore_downstream_trigger_rules=False) # skip을 그룹 내로 한정 (5.2)
            def get_table_jobs(**context):
                ...
                # 경계 기준일: loop 재trigger가 자정을 넘겨도 흔들리지 않게 conf로 고정 (5.5)
                base = (pendulum.parse(conf["base_date"], tz=KST)
                ...
                if params.get("target_dt"):            # 수동: 지정 날짜 전체 (WAIT+FAILED)
                    ...
                    if d >= base.subtract(days=2):     # 그저께 이전만 허용 (5.1)
                        raise ValueError("target_dt는 그저께 이전 날짜만 허용 — append 조회 범위와 겹침")
                ...
                fetched_full = len(jobs) >= ROW_LIMIT   # 잔여분 판정은 필터 전 건수 기준 (5.3)
                ...
                # job_ids로 회수 가능 (좀비 IN_PROGRESS 경로 축소, 5.3)
            ...
            picked >> append_data
            # 상태 update는 XCom meta의 job_ids로 UPDATE (stat_desc 조건 금지 — 섹션 4.2)
            append_data >> update_success_task(tbl)    # [all_success] job_ids → DONE
            append_data >> update_failure_task(tbl)    # [all_failed]  job_ids → FAILED
        return group
    ...
    def check_loop(**context):
        # 재trigger 조건: 잔여분 존재 AND 그 테이블의 이번 회차 Spark 성공 (섹션 5.5)
        # 실패 테이블은 제외 — 같은 밤에 깨진 데이터를 반복 재시도하지 않음 (다음날 회수)
        pending = [m for m in collect_group_metas(context)
                   if m["leftover"] and spark_succeeded(context, m["table"])]
    ...
    retrigger = TriggerDagRunOperator(
        ...
        conf={
            ...
            # 경계 기준일 고정 전달 — 회차가 자정을 넘겨도 ts 경계 유지 (섹션 5.5)
