...
| **커스텀 객체는 XCom을 못 건넌다** | `DriverAndExecutor` 인스턴스를 `compaction_specs`가 직접 반환하면 직렬화에서 걸린다. XCom에는 원시 타입만 보내고 객체 생성은 `.map()`이 XCom **이후에** 수행한다. `.map()`에 넘기는 함수는 `@task`가 아니어야 한다. 그 외의 조립은 Enum이 손에 있는 `compaction_specs`에서 끝내는 편이 낫다 — 이름만 보내고 `.map()`에서 Enum을 되찾으면 한 바퀴 돌 뿐 아니라 `get_name()`이 Enum value와 다를 때 깨진다 |
