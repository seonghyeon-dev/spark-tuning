...
적용 범위
  **hourly 전용이다.** daily Compaction은 아직 튜닝하지 않았고 대상 테이블·`rewrite-all`
  전제가 미확인이라(guide §8.1) 계수 C를 그대로 쓸 수 없다.
"""
...
#
# ⑤ daily에 그대로 쓰지 말 것
#    C=0.32은 hourly 측정값이다. daily는 대상 테이블·rewrite-all 전제가 미확인이라
