...
| 결론 | **Spark Dynamic Allocation + `executorAllocationRatio` 채택.** 1번 테이블 7회(섹션 5.1) + 2번 테이블 8회(섹션 5.2) 실측 검증. **`spark.executor.instances` = `initialExecutors` = `minExecutors` 필수** (섹션 4.8) |
...
### 5.2 2번 테이블 (하루 576GB, 시간당 약 24GB) — 8회 측정, 8대 + 20g 확정
...
출력 파일 품질은 8회 전부 정상 — 45~49개, avg 500~510MB, max 550~591MB. min은 test2의 275.8MB 한 건을 빼면 386~435MB.
...
| 운영 적용 후 duration | 운영에서는 `initialExecutors`가 수렴 대수와 같아 6 → 8 warm-up이 없으므로 test8의 1.9분보다 짧을 것으로 예상. 적용 후 확인 |
