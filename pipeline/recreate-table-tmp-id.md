...
spark.sql(s"INSERT INTO $Tbl SELECT $cols FROM $Tmp t LEFT JOIN ora o ON $JoinOn WHERE $Where")
// 변경 후: tmp_id 가 '' 이고 Oracle 에 키가 있는 row 만 UPDATE (max() 는 키당 1건임을 Spark 에 알리는 포장 — 유일성은 ora 키 중복 검수가 보장)
spark.sql(s"UPDATE $Tbl t SET tmp_id = (SELECT max(o.tmp_id) FROM ora o WHERE $JoinOn) WHERE $Where AND t.tmp_id = '' AND EXISTS (SELECT 1 FROM ora o WHERE $JoinOn AND o.tmp_id IS NOT NULL)")

// 변경 전: 사후 검수 ②
assertZero("tmp_id 불일치", s"SELECT COUNT(*) FROM $Tbl t LEFT JOIN ora o ON $JoinOn WHERE $Where AND NOT (t.tmp_id <=> COALESCE(o.tmp_id, ''))")
// 변경 후: Oracle 에 키가 있는데 '' 로 남은 row 가 0 인지 (다른 Oracle 로 돌릴 때 앞서 채운 값은 이 Oracle 에 없으므로 불일치 검수는 못 쓴다)
